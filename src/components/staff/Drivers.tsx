...
function DriverProfileModal({ driver, onClose }: { driver: Driver | undefined, onClose: () => void }) {
...
function DriverHistoryModal({ driver, onClose }: { driver: Driver | undefined, onClose: () => void }) {
    if (!driver) return null;
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm animate-in fade-in zoom-in-95 duration-200">
...
function EditDriverModal({ driver, onClose, onSave, onDelete }: { driver: Driver | undefined, onClose: () => void, onSave: (d: Driver) => void, onDelete: (id: string) => void }) {
    if (!driver) return null;
